...
const cssImportantifyString = (string)=>{
    const cssStringPattern = /"(?:\\.|[^"])*"|'(?:\\.|[^'])*'/g
    const cssStrings = string.match(cssStringPattern)||[]
    const nonCssStringParts = string.split(cssStringPattern)
    // add important to everything, and add the css string part back (kinda like zipping it back up)
    return nonCssStringParts.map(
        each=>each.replace(/(!important)?[ \t\n\r]*;/g, " !important;")+(cssStrings.shift()||"")
    ).join("")
}
